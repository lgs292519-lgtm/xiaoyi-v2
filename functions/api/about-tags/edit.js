// 管理员：编辑固定/备用标签名称
// POST /api/about-tags/edit { tagType: 'fixed'|'extra', id, name, adminPass? }
...
export async function onRequest(context) {
  ...
  try {
    await db.prepare(`UPDATE ${table} SET name = ? WHERE id = ?`).bind(name, tagId).run()
  } catch (e) {
    return json({ ok: false, error: 'name conflict' }, { status: 409 })
  }
